...
y1(i)=(2*sqrt(eps)*(x(i)+3*exp(-(x(i)*x(i)-1)/(2*eps)))+exp(1/(2*eps))*sqrt(2*pi)*x(i)*erf(1/sqrt(2*eps))+3*exp(1/(2*eps))*sqrt(2*pi)*x(i)*erf(x(i)/sqrt(2*eps)))/(2*exp(1/(2*eps))*sqrt(2*pi)*erf(1/sqrt(2*eps))+4*sqrt(eps));
...
toc%maxh1=max(zh1)';
N=[2^(7),2^(8),2^(9),2^(10)];
E1=[4.0592e-07, 2.5378e-08, 1.5863e-09, 9.9321e-11];
E2=[1.1498e-06, 7.1811e-08, 4.4873e-09, 2.8046e-10];
E3=[3.2550e-06, 2.0296e-07, 1.2689e-08, 7.9318e-10];
E4=[9.2166e-06, 5.7490e-07, 3.5906e-08, 2.2437e-9];
%plot(x, y2)
loglog(N, E4,'-*')
